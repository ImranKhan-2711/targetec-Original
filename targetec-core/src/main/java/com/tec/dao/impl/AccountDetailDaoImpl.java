...
public class AccountDetailDaoImpl extends NeuroGenericDAO implements AccountDetailDao {
	...
	public AccountDetail insert(AccountDetail accountDetail) {
		...
		jdbcTemplate.update(insertSQL,
				new Object[] { accountDetail.getAccount().getId(), accountDetail.getFirstName(),
						accountDetail.getLastName(), accountDetail.getEmail(), accountDetail.getPhoneNumber(),
						accountDetail.getProfileImage().getId() });
	...
	public Page<AccountDetail> findStudentsfindStudentsWithoutTestId(Page<AccountDetail> page, String firstName,
		...
		System.out.println(selSQL + "jdbcTemplate\n\n\n");
		String countSQL = "select count(*) from (" + selSQL + ") c";
		return ph.fetchPage(jdbcTemplate, countSQL, selSQL, new String[] {}, page, new AccountDetailMapper());
	}
	...
	public AccountDetail getAccountByUserName(String userName) {
		PaginationHelper<AccountDetail> ph = new PaginationHelper<AccountDetail>();
		Page<AccountDetail> page = new Page<AccountDetail>("");
		String selSQL = "select acd.id,acd.first_name,acd.last_name,acd.email,acd.phone_number, rsm.resource_ext, rsm.resource_path,acd.account_id,a.username,rsm.id rsm_id from account_detail acd, resource_master rsm,account a where acd.account_id=a.id and acd.profile_image=rsm.id and acd.account_id in (SELECT id from account where username= ?) ";
		String countSQL = "select count(*) from (" + selSQL + ") c";
		return ph.fetchPage(jdbcTemplate, countSQL, selSQL, new Object[] { userName }, page, new AccountDetailMapper())
				.getPageItems().get(0);
	}
}
